...
2026-09-09 로컬 변경: 첫 화면은 가족별 **나만의 스튜디오**다. 우선 활동을 선택해 들어가며 강제 이동하지 않는다. 아래 과거 로그의 세션당 리다이렉트 설명은 이전 동작이다.
...
- **하울** — 대학 가이드 「확인」 탭. ① 관심 대학 입학처에 **검정고시 출신이 그 국제계열
  ...
  (031-971-2731)에 **Grade 10인 하울의 졸업 예정일과 남은 이수 학점** 확인.

- **하음** — ① 하음이 **일반고인지** 확인 (아니면 검정고시가 먼저다) ② 지원할 **대학과 전형**을
  먼저 정해야 실기 연습 방향이 잡힌다 (일반대 수시 6회 제한, 전문대 제외)
...
- **하울 진로 가이드플랜** — `docs/하울_진로_조사.md` 의 「5. 학교에 확인해야 할 것」 7가지를
  ...
  하울의 미국 학년은 **Grade 10**으로 보호자가 확인했다(2026-09-09). 졸업일·목표 대학 입학연도는 별도 확인한다.
...
| 요리 그림 | `src/components/FoodArt.jsx` | 2026-09-09 16개 일러스트 적용. 개별 image가 있으면 사진 우선 |
...
- 하음 웹툰 **면접 탭**(커밋 `6eec919`) origin/main push 완료 → Vercel 배포.
- 빌드 통과. (Supabase 로그인 제한은 별개 — DB 1.1GB→18MB로 원인 제거했으나 "청구기간 평균" 기준이라 자동 복구 대기 중.)

### 2026-09-19 — 하음 웹툰 면접 준비 탭 신설 + 학교별 심층(청강·한국영상대·백석)

- **요청**: 웹툰학과 면접 대비 자료 수집 + 가이드, 특히 청강대 중심 + 백석예술대·한국영상대 심층.
- **조치(코드)**:
  - `data/webtoon.js`: `INTERVIEW`(공통 면접: 진행방식·5축·실제기출12·평가·감점·팁·디펜스워크시트·체크리스트) + `SCHOOL_INTERVIEWS`(학교별 심층) 추가. `tutorContext()`에 면접 요약 보강.
  - `pages/Webtoon.jsx`: 탭에 「💬 면접」 신설(`InterviewTab`). 기출 아코디언, 디펜스 워크시트(textarea)·체크리스트는 **localStorage 개인저장**(lsGet/lsSet, try/catch). 학교 카드는 `SchoolInterviewCard`.
- **리서치(웹, 2026 요강·후기 기반)**: 청강대(3부스·20분 즉석드로잉·A3 25~30p·이름/사인금지·실제기출), 한국영상대 만화웹툰전공(면접60+교과40, 실기 택1 4시간, 10분 구술 4항목, 내신 인사이트), 백석예술대(서울)=영상학부 만화애니메이션전공(‘웹툰과’ 없음, 실기·예약 10/13), 백석문화대(천안)=실제 ‘웹툰과’(실기 4절 3시간 유채, 기출 주제).
- **⚠️ 주의(다음 사람/보호자 확인)**: 서울 **백석예술대엔 ‘웹툰과’가 없다**(만화애니메이션전공). 진짜 ‘웹툰과’는 **백석문화대(천안)**. 하음이 지원할 곳이 어디인지 먼저 확정. 각 수치·일정은 2026 기준이라 **2027 요강 재확인** 필요.
- **검증**: `frontend npm run build` 통과. 데이터는 참고용, 최종은 각 입학처 요강.
- **커밋/배포**: (아래 커밋). main push·Vercel 반영은 보호자 확인 후.
...
- **원인**: main 커밋·push 미실행. 이전 배포 시도는 자동 승인 검토의 사용량 한도로 중단.
- **조치**: 원격 main 동기 상태 확인 후 스튜디오·입시 자료·Grade 10 변경을 함께 커밋하여 main 배포 경로로 반영.
- **검증**: Node 회귀 테스트 3개 통과. production build 및 실제 서비스의 정적 번들 갱신 확인 후 결과를 추가 기록.
- **커밋**: 이 로그를 포함하는 스튜디오 적용 커밋. AGENTS.md는 별도 사용자 파일이므로 제외.
- **남은 것**: 실제 계정별 DB 저장·AI 응답은 가족 로그인 후 확인. 학교 서류 및 개인 지원 연도 미확정 사항은 기존 기록 유지.


### 2026-09-09 — 하울 미국 Grade 10 확인

- 보호자 확인: 하울은 미국 학년 Grade 10. 대학 가이드 표시·입시 AI 컨텍스트·조사 기록에 반영.
- 기존 학년 확인 질문은 졸업 예정일·남은 학점 확인으로 변경. 졸업연도나 대학 입학연도는 추정하지 않음.
- 자료 버전 v2로 갱신. 로컬 수정이며 커밋·push·배포 전.

### 2026-09-09 — 나만의 스튜디오 실제 적용 + 2027 입시 원문 검증 (로컬)

- **요청/원인**: 시안 B를 실제 앱에 적용. 하음 웹툰 입시, 하울 글로벌 대학, 하람 요리를 먼저 보여주고 기존 활동을 보존.
- **조치**: `StudioHome` 개인 홈, 밝은 공통 테마, 입체 카드/버튼, PC 사이드 메뉴·모바일 하단 메뉴. 부모 전체/자녀 허용 기능 규칙 유지. 초기 강제 리다이렉트 대신 스튜디오에서 선택.
- **요리**: 16개 음식 일러스트 원본을 `public/images/food-studio.png`로 추가하고 CSS로 각 요리를 표시. 기존 저장함·재료·조리 단계 유지. 요거트 컵은 미리 손질한 과일/어른 도움으로 칼 안내 모순 수정.
- **입시**: `admissions2027.js` + `AdmissionGuide`. 연세/고려(9월 8일 수정본), 공주/동서 2027 요강 반영. 청강은 공식 Q&A 검색 확인, 전체 원문 미검증 표시. 확인일·원문 링크·준비 단계·가족별 로컬 체크 추가.
- **정정**: 근거 없는 어학 점수선, 글로벌 전형 일반화, 전문대까지 수시 6회로 제한, 포트폴리오 공통 필수 컷 수 수정. 미국 인증/졸업장/국내 학력/해외고 자격 구분. MSA 2022 5년 인증 목록에서 LIS Korea 확인했으나 현재 인증과 졸업장 발급기관은 미확정.
- **AI**: 최신 검증 자료를 컨텍스트로 전달하고 클라이언트 topic에 자료 버전을 넣어 이전 입시 답변 캐시와 분리. Edge Function 수정/배포 없음.
- **검증**: Node 회귀 테스트 3개, production build 통과. 실컴포넌트 미리보기에서 390px/1366px, 부모19·하음12·하울16·하람13 활동, 레시피 저장/단계 체크, 대학 상세, 콘솔 오류 없음 확인. 실제 계정 DB 저장과 유료 AI 응답은 미실행.
- **미리보기**: `cd frontend` → `npx vite --config dev/vite.config.mjs` → `http://127.0.0.1:5174/dev/studio-preview.html`. 개발 전용 예시 프로필이며 운영 빌드에 포함되지 않음.
- **근거 문서**: `docs/research/2026-09-09/README.md`. 기존 시안은 `docs/design/2026-09-studio/`.
- **커밋/배포**: 아직 로컬 수정. 커밋·push·외부 배포 안 함. 새 SQL 없음. 기존 미추적 `AGENTS.md`는 수정/커밋하지 않음.
- **남은 확인**: 하음/하울 목표 입학연도, 하울 졸업일(미국 Grade 10은 보호자 확인 완료), ‘sit’=SAT 여부, LIS 자체 SAT 졸업요건, 현재 인증서/졸업장 발급기관. 학교 자료와 입학처 회신으로 개인 자격 확정. 청강 전체 요강 재대조.
